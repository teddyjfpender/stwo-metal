...
pub trait PolyOps: ColumnOps<BaseField> + ColumnOps<SecureField> + Sized {
    ...
    /// Evaluates multiple same-size polynomials at the same point.
    ///
    /// The default implementation preserves the single-point semantics by delegating to
    /// [`Self::eval_at_point`] per polynomial. Backends may override this to batch the work on
    /// hardware.
