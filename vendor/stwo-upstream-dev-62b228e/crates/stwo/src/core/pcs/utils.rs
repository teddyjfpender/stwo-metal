...
pub fn prepare_query_positions_for_tree(
    query_positions: &[usize],
    max_log_size: u32,
    tree_max_log_size: u32,
) -> Vec<usize> {
    if tree_max_log_size == 0 {
        return vec![];
    };
    if max_log_size < tree_max_log_size {
        return query_positions
            .iter()
            .map(|pos| (pos >> 1 << (tree_max_log_size - max_log_size + 1)) + (pos & 1))
            .collect();
    }
    query_positions
        .iter()
        .map(|pos| (pos >> (max_log_size - tree_max_log_size + 1) << 1) + (pos & 1))
        .collect()
}
...
pub fn prepare_preprocessed_query_positions(
    ...
    prepare_query_positions_for_tree(query_positions, max_log_size, pp_max_log_size)
}
