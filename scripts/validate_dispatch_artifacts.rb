...
if ARGV.length != 2
  warn "usage: ruby scripts/validate_dispatch_artifacts.rb <registry.json> <generated_dispatch.cuh>"
  exit 1
end
...
generator_path = repo_root.join("scripts/generate_dispatch_registration.rb")

Tempfile.create(["dispatch-registration", ".cuh"]) do |tempfile|
  ...
  if expected != actual
    warn "dispatch artifact drift detected in #{generated_path}"
    exit 1
  end
end

puts "dispatch artifacts ok"
