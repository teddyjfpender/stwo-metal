...
log_n_instances="${STWO_BENCH_LOG_N_INSTANCES:-22}"
...
if [ "$plan_only" != "1" ] && [ "$mode" = "no-cuda" ]; then
    echo "poseidon_prove benchmark requires STWO_CUDA_MODE != no-cuda unless STWO_BENCH_PLAN_ONLY=1." >&2
    exit 1
fi
...
command_string="cargo run --release --manifest-path fixtures/standalone-benchmarks/Cargo.toml --bin poseidon_prove $runtime_feature_args"
