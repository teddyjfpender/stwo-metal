...
usage() {
  cat <<'EOF'
usage: run_remote_upstream_fri_validation.sh <host>
...
and runs the narrow upstream FRI validation under STWO_CUDA_MODE=cuda-ci.
EOF
}
...
echo "== upstream fri candidate row =="
STWO_UPSTREAM_FRI_VALIDATE_MODE=test \
  sh scripts/run_standalone_upstream_fri_validation.sh
EOF
