...
RUNPOD_REMOTE_ENV="${RUNPOD_REMOTE_ENV:-STWO_CUDA_MODE=cuda-dev}" \
  bash "$repo_root/scripts/run_runpod_remote.sh" \
    --pod-name "${RUNPOD_POD_NAME:-stwo-cuda-m45-poseidon-point-values}" \
    --log-label "poseidon-direct-point-values" \
    -- \
    cargo test --manifest-path fixtures/standalone-benchmarks/Cargo.toml \
      --bin poseidon_prove \
      tests::supported_poseidon_direct_trace_point_values_match_cpu_reference \
