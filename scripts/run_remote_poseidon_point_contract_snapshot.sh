...
usage() {
  cat <<'EOF'
usage: run_remote_poseidon_point_contract_snapshot.sh <host>
...
Environment:
  ...
  REMOTE_DIR  Remote checkout path. Default: /root/stwo-cuda or /home/<user>/stwo-cuda
  BOOTSTRAP   Run remote bootstrap first. Default: 1

This script syncs the current repository to a remote CUDA-capable host, runs the
supported Poseidon prove-core snapshot characterization lane under
STWO_CUDA_MODE=cuda-dev, and captures the combined remote log locally.
EOF
}
...
log_dir="${repo_root}/logs/remote-cuda/${timestamp}-poseidon-point-contract"
...
ssh "${ssh_opts[@]}" "${remote}" "bash -s -- $(printf '%q' "${remote_dir}")" 2>&1 <<'EOF' | tee "${log_file}"
set -euo pipefail
...
export STWO_CUDA_MODE="cuda-dev"
export STWO_CUDA_DEBUG_PROVE_VALUES="1"
...
echo "== poseidon point-contract snapshot =="
...
echo
echo "remote snapshot log saved to ${log_file}"
