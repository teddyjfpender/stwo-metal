...
usage() {
  cat <<'EOF'
usage: run_remote_upstream_accumulation_validation.sh <host>
...
and runs the narrow upstream accumulation validation under STWO_CUDA_MODE=cuda-ci.
EOF
}
...
echo "== upstream accumulation candidate row =="
STWO_UPSTREAM_ACCUMULATION_VALIDATE_MODE=test \
  sh scripts/run_standalone_upstream_accumulation_validation.sh
EOF
