...
usage() {
  cat <<'EOF'
usage: run_remote_upstream_quotient_validation.sh <host>
...
and runs the vendored-upstream quotient validation under STWO_CUDA_MODE=cuda-ci.
EOF
}
...
echo "== upstream quotient host-safe contract =="
STWO_CUDA_MODE=no-cuda \
STWO_UPSTREAM_QUOTIENT_VALIDATE_MODE=test \
  sh scripts/run_standalone_upstream_quotient_validation.sh
echo

echo "== upstream quotient cuda integration =="
cargo test -p stwo-cuda --test upstream_quotient_accumulate \
  --features="prover" \
  --locked \
  -- --exact --nocapture
cargo test -p stwo-cuda --test upstream_quotient_combine \
  --features="prover" \
  --locked \
  -- --exact --nocapture
EOF
