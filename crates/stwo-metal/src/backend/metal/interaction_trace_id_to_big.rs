...
/// Metal-native variant of [`gpu_gen_big_memory_interaction_trace`].
///
/// Returns `Vec<CircleEvaluation<MetalBackend, ...>>` with non-prefix-sum columns
/// kept on-GPU (zero readback for 28 of 32 M31 columns). Only the last 4 columns
/// (prefix-sum finalization) are read back to CPU, processed, and re-uploaded.
pub fn gpu_gen_big_memory_interaction_trace_metal(
    limb_columns: &[Vec<PackedBaseField>; 28],
    multiplicities: &[PackedBaseField],
    lookup_elements: &InteractionLookupElements,
    relation_ids: &InteractionRelationIds,
...
> {
    ...
    // Columns 0-6 (28 M31 columns): zero-copy from GPU buffer via clone_range.
    ...
    // Column 7 (last 4 M31 columns): read back for prefix-sum finalization.
    {
        let last_logup = n_logup_cols - 1;
        let base_m31_col = last_logup * 4;

        // Read only the 4 coordinate columns for the last logup column.
        let mut coord_columns: [stwo::prover::backend::simd::column::BaseColumn;
            SECURE_EXTENSION_DEGREE] = std::array::from_fn(|coord| {
            let col_idx = base_m31_col + coord;
            let gpu_offset = col_idx * n_rows;
            // Read this single column from GPU.
...
/// Metal-native variant of [`gpu_gen_small_memory_interaction_trace`].
///
/// Returns `Vec<CircleEvaluation<MetalBackend, ...>>` with non-prefix-sum columns
/// kept on-GPU. Only the last 4 columns (prefix-sum finalization) are read back.
pub fn gpu_gen_small_memory_interaction_trace_metal(
    limb_columns: &[Vec<PackedBaseField>; 8],
    multiplicities: &[PackedBaseField],
    lookup_elements: &InteractionLookupElements,
    relation_ids: &InteractionRelationIds,
...
> {
    ...
    // Columns 0-1 (8 M31 columns): zero-copy from GPU buffer via clone_range.
    for logup_col in 0..(n_logup_cols - 1) {
        let base_m31_col = logup_col * 4;
        for coord in 0..SECURE_EXTENSION_DEGREE {
            let col_idx = base_m31_col + coord;
    ...
    // Last column (4 M31 columns): read back for prefix-sum finalization.
    {
        let last_logup = n_logup_cols - 1;
        let base_m31_col = last_logup * 4;
