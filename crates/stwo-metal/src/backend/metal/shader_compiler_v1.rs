...
pub fn compile_v1_to_metal_source(program: &OwnedMetalEvaluationProgramV1) -> String {
    ...
    // ── Base instructions ──────────────────────────────────────────────────
    //
    // Each base instruction writes to a local `uint b<dst>`.  We declare
    // variables on first write.
    ...
    // ── Ext instructions ───────────────────────────────────────────────────
    //
    // Each ext instruction writes to a local `StwoMetalQm31 e<dst>`.
    ...
    //
    // Unroll the constraint accumulation loop.  Each constraint root is an
    // ext register whose value is multiplied by the corresponding random
    // coefficient power and accumulated.
