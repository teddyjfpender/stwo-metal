...
use crate::backend::cuda::secure_column::CudaSecureColumn;
use crate::stwo_cuda as interface;

fn fold_line_once_cuda(
...
fn fold_line_with_step_cuda(
    ...
    let mut result = fold_line_once_cuda(eval, folding_alpha, twiddles);
    for _ in 1..fold_step {
        folding_alpha = folding_alpha * folding_alpha;
        result = fold_line_once_cuda(&result, folding_alpha, twiddles);
    }
    result
}

fn fold_circle_into_line_cuda(
    dst: &mut LineEvaluation<CudaBackend>,
    src: &SecureEvaluation<CudaBackend, BitReversedOrder>,
    alpha: SecureField,
    twiddles: &TwiddleTree<CudaBackend>,
) {
    let n = src.len();
...
impl FriOps for CudaBackend {
    ...
    fn fold_line(
        ...
        fold_line_once_cuda(eval, alpha, twiddles)
    }
    ...
    fn fold_line(
        ...
        fold_line_with_step_cuda(eval, alpha, twiddles, fold_step)
    }
    ...
    fn fold_circle_into_line(
        ...
        fold_circle_into_line_cuda(dst, src, alpha, twiddles)
    }
    ...
    fn decompose(
        ...
        // it.
        panic_for_unsupported_surface(CudaBackendSurface::FriDecompose, "FriOps::decompose")
    }
}
...
mod tests {
    ...
    fn gpu_fold_line(
        ...
        CudaBackend::fold_line(eval, alpha, twiddles)
    }
    ...
    fn test_fold_line_more_than_once() {
        ...
        let first_cpu_fold = cpu_fold_line(
            &LineEvaluation::new(
                domain,
                SecureColumnByCoords {
                    columns: vec.clone(),
